...
public class RingtonePickerDialogController extends DialogFragmentController<RingtonePickerDialog> {
    ...
    public void show(Uri initialUri, String tag) {
        RingtonePickerDialog dialog = null;
        if (mVolume > 0) {
            dialog = RingtonePickerDialog.newInstance(mListener, initialUri, mVolume);
        } else {
            dialog = RingtonePickerDialog.newInstance(mListener, initialUri);
        }
        show(dialog, tag);
    }
